...
function parseAuthError(err: any): string {
  ...
  if (err?.message?.includes('TIMEOUT')) return 'Connection timed out. Check your network.'
...
export default function LoginPage() {
    ...
    <>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <Input
          ...
          required
        />
        {error && <p className="text-sm text-red-400 bg-red-950/40 border border-red-800/40 rounded-xl px-3 py-2">{error}</p>}
  ...
  return (
    ...
    >
      {/* ── Mobile: hero header ── */}
      <div className="md:hidden flex flex-col items-center justify-center flex-1 px-6 pb-4">
        <div className="w-20 h-20 rounded-3xl bg-indigo-600 flex items-center justify-center mb-5 shadow-2xl shadow-indigo-500/40">
      ...
      {/* ── Mobile: form (no card) ── */}
      <div className="md:hidden w-full px-6 pb-8">
        <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-1">Sign In</h2>
      ...
      <div className="hidden md:block w-full max-w-sm p-8 bg-zinc-50 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-sm mb-6">Sign in to your account</p>
        {formContent}
      </div>
    </div>
  )
